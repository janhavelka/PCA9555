...
- Serial RTS: `0`
- I2C address: `0x20`
- Timeout override: `per-command defaults`
- Startup timeout: `20.0`
- Idle timeout: `1.0`
- Allow idle completion: `False`
- Boot settle: `2.0`
- Serial reopen interval: `180.0`
...
| `outputs` | `read` | Read output latch registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
| `pattern 0xAAAA confirm` | `output_pattern` | Drive an exact 16-bit PCA9555 output pattern. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
| `outputs` | `read` | Read output latch registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
...
| `outputs` | `read` | Read output latch registers. | `PASS` | `N/A` | `prompt` | 0.01s |  |
...
| `polarity` | `read` | Read polarity inversion registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
| `rreg 0` | `register_read` | Read one PCA9555 register. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command. |
...
| `health` | `health` | Capture driver health before stress. | `PASS` | `N/A` | `prompt` | 0.02s | Historical last-error fields may be nonzero; current READY state is the key gate. |
| `soak:aggregate` | `soak` | Aggregate statistics for soak. | `FAIL` | `N/A` | `failure_limit` | 317.80s | Generated by bounded repeated-command runner. |

## Aggregate Timing

| Label | Completed | Failures | Serial Reopens | Min | Mean | Max | Effective Hz | Stop Reason |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `soak` | 5741 | 1 | 1 | 0.000s | 0.004s | 5.062s | 18.065 | `failure_limit` |
...
- `completed=5741`
- `failures=1`
- `result_counts={"PASS": 5740, "TIMEOUT": 1}`
- `effective_hz=18.065`
- `stop_reason=failure_limit`

## Artifacts

- Serial transcript: `hil_logs\full_function_4h_20260624\i2c_20260624_125345\serial_transcript.txt`
- Operator checklist: `hil_logs\full_function_4h_20260624\i2c_20260624_125345\operator_checklist.md`
- Machine summary: `hil_logs\full_function_4h_20260624\i2c_20260624_125345\summary.json`
