...
public:
  ...
  /// Advances one active chunked job instruction, if a job is active.
  ...
  /// instruction. Chunked jobs do not acquire Config::i2cLock across polls;
  /// callers that share a bus should serialize at the owning task.
  /// @return Err::IN_PROGRESS when scheduled, or an error when not startable
  Status startReadInputsJob();

  /// Start a chunked masked output-latch update job.
  /// Applies value bits selected by mask to the cached output shadow and writes
  /// the output register pair as one instruction. No I2C occurs when mask
  /// causes no change.
  ...
  ///         or an error when not startable
  Status startWriteOutputsJob(uint16_t mask, uint16_t value);

  /// Start a chunked safe output-configuration job.
  /// Preloads the output latch for selected input-to-output transitions, then
  /// clears the selected configuration bits. Each register-pair write is one
  /// instruction; no-op writes are skipped without consuming instruction budget.
  ...
  /// Apply the TI interrupt errata workaround under the optional bus lock.
  ...
  /// Apply the TI interrupt errata workaround without acquiring Config::i2cLock.
  /// Use only when the caller already owns bus serialization or is the only bus
  /// owner. This is the one-instruction primitive used by chunked jobs.
  /// @return Status::Ok() on success
