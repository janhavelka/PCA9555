...
| `dport 1 0xFF confirm` | `direction` | Set one port direction register. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
| `config` | `read` | Read configuration registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
...
| `wreg 3 0x33 confirm` | `register_write` | Write one writable PCA9555 register. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
| `outputs` | `read` | Read output latch registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
...
| `rregs 2 2` | `register_read` | Read one or two PCA9555 registers with pair wrapping. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command. |
...
| `stress_mix 100 confirm` | `stress_mix` | Mixed read/write/config/polarity/mask stress test. | `PASS` | `N/A` | `prompt` | 0.05s | Mixed stress drives outputs and changes configuration; opt-in only. |
...
| `recover confirm` | `recovery` | Run manual driver recovery and reapply cached state. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
...
| `health` | `health` | Capture driver health before stress. | `PASS` | `N/A` | `prompt` | 0.00s | Historical last-error fields may be nonzero; current READY state is the key gate. |
| `soak:aggregate` | `soak` | Aggregate statistics for soak. | `SERIAL_OK_OR_REVIEW` | `N/A` | `failure_limit` | 45.30s | Generated by bounded repeated-command runner. |

## Aggregate Timing

| Label | Completed | Failures | Serial Reopens | Min | Mean | Max | Effective Hz | Stop Reason |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `soak` | 426 | 0 | 0 | 0.000s | 0.005s | 1.078s | 9.405 | `failure_limit` |
...
- `completed=426`
- `failures=0`
- `result_counts={"PASS": 425, "SERIAL_OK_OR_REVIEW": 1}`
- `effective_hz=9.405`
- `stop_reason=failure_limit`

## Artifacts

- Serial transcript: `hil_logs\full_function_4h_20260624_retry2\i2c_20260624_130300\serial_transcript.txt`
- Operator checklist: `hil_logs\full_function_4h_20260624_retry2\i2c_20260624_130300\operator_checklist.md`
- Machine summary: `hil_logs\full_function_4h_20260624_retry2\i2c_20260624_130300\summary.json`
