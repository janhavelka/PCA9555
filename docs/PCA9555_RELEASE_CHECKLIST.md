...
- Version bump decision: pending
- Hardware validation: not run in Prompt 07
...
| Gate | Command / Evidence | Required Result | Current Prompt 07 Status |
| --- | --- | --- | --- |
| Branch and dirty-state review | `git status --short` and code review | Only intended hardening files are changed | Related Prompt 01-07 work present |
| Core framework guard | `python tools/check_core_timing_guard.py` | PASS | Run during Prompt 07 validation |
| Generated version check | `python scripts/generate_version.py check` | PASS | Run during Prompt 07 validation |
| ESP-IDF example contract guard | `python tools/check_idf_example_contract.py` | PASS | Run during Prompt 07 validation |
| Native tests | `python -m platformio test -e native` | PASS | Run during Prompt 07 validation |
| Arduino ESP32-S2 compile | `python -m platformio run -e esp32s2dev` | PASS | Run during Prompt 07 validation |
| Arduino ESP32-S3 compile | `python -m platformio run -e esp32s3dev` | PASS | Run during Prompt 07 validation |
| Documentation updated | README, Doxygen comments, register reference, reports | Prompt 01-07 behavior documented honestly | Updated in Prompt 07 |
| Required reports present | `docs/PCA9555_HARDENING_PROMPT_*_REPORT.md` | Reports for Prompts 01-07 present | Prompt 07 report required before merge |
| Release gates present | This file | Merge, RC, and production-claim gates documented | Added in Prompt 07 |
| Hardware matrix present | `docs/PCA9555_HARDWARE_VALIDATION_MATRIX.md` | All required tests listed and unrun tests marked honestly | Added in Prompt 07 |
...
| Pure ESP-IDF build | `idf.py -C examples/esp_idf/basic set-target esp32s3 build` and `idf.py -C examples/esp_idf/basic set-target esp32s2 build` or CI logs | PASS, or explicitly marked pending for RC |
