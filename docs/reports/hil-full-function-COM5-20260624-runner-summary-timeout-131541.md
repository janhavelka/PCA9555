...
| `probe` | `probe` | Run the driver raw address probe without health tracking. | `PASS` | `N/A` | `prompt` | 0.01s | Probe is ACK-only and does not prove chip identity. |
...
| `setbits 0x00FF confirm` | `mask_write` | Set masked output latch bits high. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
| `outputs` | `read` | Read output latch registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
...
| `polarity` | `read` | Read polarity inversion registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
| `invertclr 0x0F0F confirm` | `polarity` | Disable masked input polarity inversion. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
| `polarity` | `read` | Read polarity inversion registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
...
| `config` | `read` | Read configuration registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
| `wregs 6 0x00 0x00 confirm` | `register_write` | Write one or two writable PCA9555 registers with pair wrapping. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
| `polarity` | `read` | Read polarity inversion registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
...
| `health` | `health` | Capture driver health before stress. | `PASS` | `N/A` | `prompt` | 0.00s | Historical last-error fields may be nonzero; current READY state is the key gate. |
| `soak:aggregate` | `soak` | Aggregate statistics for soak. | `FAIL` | `N/A` | `failure_limit` | 2468.53s | Generated by bounded repeated-command runner. |

## Aggregate Timing

| Label | Completed | Failures | Serial Reopens | Min | Mean | Max | Effective Hz | Stop Reason |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `soak` | 2453 | 1 | 0 | 0.000s | 0.006s | 8.032s | 0.994 | `failure_limit` |
...
- `completed=2453`
- `failures=1`
- `result_counts={"PASS": 2452, "TIMEOUT": 1}`
- `effective_hz=0.994`
- `stop_reason=failure_limit`

## Artifacts

- Serial transcript: `hil_logs\full_function_4h_20260624_slow\i2c_20260624_131541\serial_transcript.txt`
- Operator checklist: `hil_logs\full_function_4h_20260624_slow\i2c_20260624_131541\operator_checklist.md`
- Machine summary: `hil_logs\full_function_4h_20260624_slow\i2c_20260624_131541\summary.json`
