...
| `health` | `health` | Capture driver health before stress. | `PASS` | `N/A` | `prompt` | 0.01s | Historical last-error fields may be nonzero; current READY state is the key gate. |
...
| `outputs` | `read` | Read output latch registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
| `walk 0 confirm` | `output_pattern` | Run walking-1 output pattern across all 16 pins. | `PASS` | `N/A` | `prompt` | 0.00s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
...
| `outputs` | `read` | Read output latch registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
| `clearbits 0x0F0F confirm` | `mask_write` | Clear masked output latch bits low. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
| `outputs` | `read` | Read output latch registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
...
| `outputs` | `read` | Read output latch registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
| `wport 0 0xA5 confirm` | `port_write` | Write one output port latch register. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
...
| `config` | `read` | Read configuration registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
| `dirin 0x0F0F confirm` | `direction` | Configure masked pins as inputs. | `PASS` | `N/A` | `prompt` | 0.00s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
| `config` | `read` | Read configuration registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
...
| `polarity` | `read` | Read polarity inversion registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
| `pol 15 1 confirm` | `polarity` | Set one pin input polarity inversion bit. | `PASS` | `N/A` | `prompt` | 0.02s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
| `polarity` | `read` | Read polarity inversion registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
...
| `polarity` | `read` | Read polarity inversion registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
| `rreg 0` | `register_read` | Read one PCA9555 register. | `PASS` | `N/A` | `prompt` | 0.00s | Dynamic CLI command. |
...
| `wreg 5 0xFF confirm` | `register_write` | Write one writable PCA9555 register. | `PASS` | `N/A` | `prompt` | 0.00s | Dynamic CLI command matched the unsafe output-control allowlist and requires opt-in. |
| `config` | `read` | Read configuration registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
...
| `polarity` | `read` | Read polarity inversion registers. | `PASS` | `N/A` | `prompt` | 0.00s |  |
| `selftest confirm` | `selftest` | Run CLI API self-test that mutates output, direction, and polarity state. | `PASS` | `N/A` | `prompt` | 0.03s | The CLI labels this safe, but it changes PCA9555 latches, direction, and polarity before restoring. Use only on a known-safe fixture. |
...
| `soak:aggregate` | `soak` | Aggregate statistics for soak. | `PASS` | `N/A` | `duration_limit` | 120.00s | Generated by bounded repeated-command runner. |

## Aggregate Timing

| Label | Completed | Failures | Serial Reopens | Min | Mean | Max | Effective Hz | Stop Reason |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `soak` | 1161 | 0 | 0 | 0.000s | 0.002s | 0.032s | 9.675 | `duration_limit` |

## Evidence Excerpts

## Artifacts

- Serial transcript: `hil_logs\full_function_smoke2_20260624\i2c_20260624_130516\serial_transcript.txt`
- Operator checklist: `hil_logs\full_function_smoke2_20260624\i2c_20260624_130516\operator_checklist.md`
- Machine summary: `hil_logs\full_function_smoke2_20260624\i2c_20260624_130516\summary.json`
